...
correct_num_ac = [0] * 10 #各モデルの正解数を入れる配列
accuracy_ac = [] #各モデルの正解率を入れる配列
...
def mk_input_data():
    ...
    global input_data_num2
    input_data_num2 = int(len(lines))
    print('input_data_num2' + str(input_data_num2))
...
def ann_evaluation(input_data, correct_data, model_id):
    ...
    global correct_num_ac
    correct_num_ac[model_id] = 0  # 正解数
    # output_data2 = []
    ...
    # # モデルの評価結果
    # print('正解数：' + str(correct_num[model_id]))
    # print('入力データ数：' + str(input_data_num))
    accuracy = correct_num_ac[model_id] / input_data_num
...
if __name__ == '__main__':
    ...
    for i in range(num_models):
        ...
        # モデルの評価結果
        print('正解数：' + str(correct_num_ac[i]))
        print('入力データ数：' + str(input_data_num2))
        accuracy = correct_num_ac[i] / input_data_num2

        print('モデル' + str(i) + 'の評価結果')
        print('Accuracy: ' + str(accuracy))
        with open(cir + 'model_accuracy.txt', 'a') as f:
            f.write('モデル' + str(i) + 'の正解率：' + str(accuracy) + '\n')


    print(result)
    with open(cir + 'model_accuracy_result.txt', 'a') as f:
        current_time = get_current_datetime()
        f.write(str(corrnt_time))
        f.wirte(str(result) + '\n')
# learn_data_suplit8.pyを変更したプログラム
# numpyを利用して高速化を図った．
...
#　実行コマンド：　python3 learn_data_suplit8.py
...
# --- 高速版：NumPy + 差分計算 ---
def distribute_columns_greedy_fast(correct_data, target_sizes, tries=10, seed=None, balance_weight=0.5, use_range=False):
    """
    高速版：NumPy 配列を使用し、差分削除数を効率的に計算
    
    Args:
        correct_data: 2次元リスト（行=パターン、列=信号線）
        target_sizes: 各スプリットが受け取る列数リスト
        tries: ランダム初期化回数
        balance_weight: 平均削除数とスプレッドのトレードオフ係数（>=0）
        use_range: True のとき spread に max-min を使う（False は標準偏差）
    
    Returns:
        splits: 各スプリットに割り当てられた列インデックスのリスト
    """
    random.seed(seed)
    
    # NumPy 配列に変換（高速な列アクセス）
    correct_data_np = np.array(correct_data, dtype='U1')
    R, C = correct_data_np.shape
    ...
    for t in range(tries):
        ...
        # 各スプリットの行シグネチャを文字列配列で管理
        cur_sig = [np.array([''] * R, dtype='U1000') for _ in range(num_splits)]
        cur_del = np.array([R - 1] * num_splits, dtype=np.int32)
        ...
        for c in cols:
            ...
            for s in range(num_splits):
                ...
                # 新しいシグネチャを計算（文字列連結、NumPy 高速化）
                new_sig = np.char.add(cur_sig[s], correct_data_np[:, c].astype('U1'))
                
                # 削除数を計算（np.unique で高速化）
                unique_sigs, counts = np.unique(new_sig, return_counts=True)
                ...
                if best_delta is None or delta > best_delta:
                    best_delta = delta
                    best_s = s
                    best_new_sig = new_sig
                    best_new_del = new_del
                elif delta == best_delta:
                    # 同値の場合：ばらつきが大きくなる方を優先
                    cand_del_list = cur_del.copy()
                    cand_del_list[s] = new_del
                    best_del_list = cur_del.copy()
                    best_del_list[best_s] = best_new_del
                    
                    if use_range:
                        cand_spread = np.max(cand_del_list) - np.min(cand_del_list)
                        best_spread = np.max(best_del_list) - np.min(best_del_list)
                    else:
                        cand_spread = float(np.std(cand_del_list)) if len(cand_del_list) > 1 else 0.0
                        best_spread = float(np.std(best_del_list)) if len(best_del_list) > 1 else 0.0
            ...
            if best_s is None:
                # フォールバック：列数が最少のスプリット
                lengths = np.array([len(splits[i]) for i in range(num_splits)])
                best_s = int(np.argmin(lengths))
                new_sig = np.char.add(cur_sig[best_s], correct_data_np[:, c].astype('U1'))
                unique_sigs, counts = np.unique(new_sig, return_counts=True)
        ...
        if use_range:
            spread = float(np.max(cur_del) - np.min(cur_del))
        else:
            spread = float(np.std(cur_del)) if len(cur_del) > 1 else 0.0
        
        score = -mean_del + balance_weight * spread

        if best_score is None or score < best_score:
            best_score = score
            best_splits = [s[:] for s in splits]
            no_improve_count = 0
        else:
            no_improve_count += 1
        ...
        if no_improve_count >= patience:
            print(f"早期終了: スコア改善なし（{no_improve_count}/{patience}）")
            break
...
def run_single_try_hash(seed_val, val_arr, target_sizes, balance_weight=0.0, use_range=False):
    """
...
def distribute_columns_greedy_hash_parallel(correct_data, target_sizes, tries=30, seed=None, num_workers=None, balance_weight=0.0, use_range=False):
    """
...
remainder_flag = 0  # signal_sumをsuplit_numで割り切れるかどうかを示すフラグ

...
# ann_learn2.pyをベースに、先行研究と同様に1つの出力ノードが1つの信号線に対応するように正解データを分割するように変更
# cd workspace/research2/experiment
#　実行コマンド：　python3 ann_learn8.py
...
correct_data_file = cir + '分割nomal正解データ' + '/' + cir + 'integrated_nomaloutput'  # 統合後の正解データファイル
suplit_num_file = cir + '分割nomal正解データ' + '/' + cir + 'suplit_num'  # 分割数が保存されたファイル
...
epochs = 1100
batch_size = 4
model_folder = cir + 'nomalsepmodel'  # 学習済みモデルを保存するフォルダ
...
def build_and_train_model(input_data, correct_data, model_id):
    ...
    with open(cir + 'model' + str(model_id) + '_before_save_nomaloutput.txt', 'w') as f:
...
def ann_evaluation(input_data, correct_data, model_id):
    ...
    for i in range(input_data_num):
        ...
        with open(cir + 'model' + str(model_id) + '_savecorrect_nomaldata.txt', 'a') as f:

...
#　実行コマンド：　python3 re_ann_learn.py
...
model_folder = cir + 'sepmodel_ex'  # 全てのモデルを学習させる場合に、学習済みモデルを保存するフォルダ
...
not_100_percent_models_file = cir + 'not_100_percent_models'  # 正解率が100%でなかったモデルの記録ファイル
...
re_data_correct_rate_file = cir + 're_data_and_correct_rate' # 再学習後のノード数，データ数，正解率 ⇒ data_correct_rate_fileと一緒だと万一学習が失敗したときに困るので、別ファイルに保存
correct_rate_file = 'model_correct_rate.txt'  # 今までの学習記録を保存するファイル
...
single_line_inf_file = cir + 'single_line_inf'  # 統合されていない信号線の情報を保存するファイル
...
add_mid_node_num = 25  # 追加する中間層ノード数
add_mid_node_num2 = 25  # 追加する中間層ノード数2
...
def mk_output_data(fname, model_id):
    ...
    if model_id == 0:  # 最初のモデルの場合
        with open(correct_rate_file, 'a') as g:
...
def worker(id):
    ...
    return accuracy, mid_node_num, mid_node_num2
...
if __name__ == '__main__':
    ...
    with open(correct_rate_file, 'a') as g:
        print(f"学習時間：{end - start:.4f}秒", file=g)
        print(result, file=g) 
        print("平均正解率：", sum(all_correct_rate_data) / len(all_correct_rate_data), file=g)
        print("正解率100%のモデル数：", result.count(1.0), file=g)
    ...
    # 再学習後、再学習していないモデル（正解率100％のモデル）を含めたすべてのモデルのモデル情報を更新
    with open(data_correct_rate_file, 'w') as f:
        f.write(datetime.strftime("%Y-%m-%d %H:%M:%S") + "\n")
        f.write("モデル番号,  ノード数,  削除後正解データ数,  正解率\n")
        re_learning_models_count = 0
        for i in range(len(all_correct_rate_data)):
            if i == not_100_percent_models[re_learning_models_count]:
                f.write(str(i) + ",  " + str(middle_node[i][0]) + "-" + str(middle_node[i][1]) + ",  " + str(delited_data_num[i]) + ",  " + str(result[re_learning_models_count]) + "\n")
                re_learning_models_count += 1
            else:

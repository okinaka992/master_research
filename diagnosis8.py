# diagnosis7.pyを編集．
# 高速化のためにget_fault_targetを改良した


# learn_data_delite3.ipyn以降で分割したデータを用いて学習させたANNを用いて故障診断を行うプログラム
...
def get_fault_output(tp_num, fault_target_line, fault_target_type, fault_flag):
    """
    並列処理をスレッドに変更して pickling エラーを回避。
    ファイル読み込み中心ならスレッドで十分高速化できる。
    """
    from concurrent.futures import ThreadPoolExecutor
    part_file_prefix = part_stdic_file if fault_flag == 0 else part_brdic_file
    diagnosis_file_prefix = st_diagnosis_dir + 'fault_output' if fault_flag == 0 else br_diagnosis_dir + 'fault_output'
    target_num = len(fault_target_line)

    def process_one_file(i):
        path = part_file_prefix + str(i)
        try:
        ...
        if fault_flag == 0:
            ...
            for k in range(0, len(body) - 1, 2):
                ...
                try:
                    ...
                    br_type = (br_type + 1) % 10
                    continue
                mapping[(line_id, br_type)] = body[k + 1]
                br_type = (br_type + 1) % 10

        return [mapping.get((fault_target_line[t], fault_target_type[t]), '') for t in range(target_num)]

    workers = min(max(1, processes), tp_num)
    if workers == 1:
        results = [process_one_file(i) for i in range(tp_num)]
    else:
        with ThreadPoolExecutor(max_workers=workers) as exe:
            results = list(exe.map(process_one_file, range(tp_num)))

    for i in range(tp_num):
        print(f"{i}番目のテストパターンの故障出力値を保存中...")
        out_vals = results[i]
        with open(diagnosis_file_prefix + str(i), 'w') as f:
            for v in out_vals:
                f.write(str(v) + '\n')

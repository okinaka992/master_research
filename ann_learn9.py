# 複数のANNの学習を並列に行うプログラム
# ann_learn2.pyをベースに、learn_data_suplit2.pyによって正解データによって機械学習モデルのパラメータを変更できるようにしたプログラム
# cd workspace/research2/experiment
# cd workspace/research2/experiment
#　実行コマンド：　python3 ann_learn9.py
...
mid_node_num = 700  #生成器の中間層のノード数　初期値は128　学習結果によって変更
mid_node_num2 = 1000  #生成器の中間層のノード数　初期値は128　学習結果によって変更
output_node = None #　出力層のノード数＝分割数による
...
def mk_output_data(fname):
    ...
    global output_node              #グローバル変数を書き換え
    output_node = int(len(lines[0]))      #出力ノード数を設定。出力ノード数は正解データの各行の要素数
    ...
    return np.array(int_lines), output_node_num


# モデルの学習を行う関数
def build_and_train_model(input_data, correct_data, output_node_num, model_id):
    print('モデル' + str(model_id) + 'の学習を開始します')
    print(correct_data)

    # モデルの構築
    model = Sequential()
    model.add(Dense(mid_node_num, input_dim=input_node_num, activation='tanh'))
    model.add(Dense(mid_node_num2, activation='tanh'))
...
def ann_evaluation(input_data, correct_data, output_node_num, model_id):
    # モデルの読み込み
...
def worker(model_id):
    ...
    correct_data, output_node_num = mk_output_data(correct_file)
    print(model_id, correct_data, output_node_num)

    build_and_train_model(input_data, correct_data, output_node_num, model_id)

    accuracy = ann_evaluation(input_data, correct_data, output_node_num, model_id)
...
if __name__ == '__main__':
    ...
    with open('model_correct_rate.txt', 'a') as g:
        ...
        print("中間層1ノード数：" + str(mid_node_num) + "   中間層2ノード数：" + str(mid_node_num2) + "   出力ノード数：" + str(output_node) + "   エポック数：" + str(epochs) + "   バッチサイズ：" + str(batch_size), file=g)

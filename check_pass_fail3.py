...
def get_same_passfail_sum(pass_fail, fault_flag): # fault_flagは0なら縮退故障、1ならブリッジ故障を表す
    if fault_flag == 0:  # 縮退故障の診断を行う場合
        fault_output_file = st_diagnosis_dir + 'fault_output'  # 縮退故障の故障出力を保存しているファイルのパスを指定
        pass_fail_file = st_diagnosis_dir + 'pass_fail'  # 縮退故障のパスフェイル保存ファイル名
        target_fault_num = st_target_fault_num  # 縮退故障診断対象の信号線の総数を取得
    ...
    pass_fail_str = [''.join(str(x) for x in row) for row in pass_fail]  # パスフェイル情報を文字列に変換, a = [[0,1,1], [1,1,1], [0,0,1]]⇒a_str = ['011', '111', '001']
    same_counter = Counter(pass_fail_str)  # パスフェイルが同じである故障候補の数をカウント
    same_count = [same_counter[x] for x in pass_fail_str]  # 各故障候補に対するパスフェイルが同じである故障候補の数を取得
...
if __name__ == '__main__':
    ...
    get_same_passfail_sum(st_pass_fail, 0)
    get_same_passfail_sum(br_pass_fail, 1)

# diagnosis5.pyを編集．learn_data_suplit4.pyで分割したデータを用いて学習させたANNを用いて故障診断を行うプログラム
            ...
            for integ_num in range(len(correct_data_integrated_inf[i])):
                ...
                if model_id == (num_models - 1) and single_flag == 1:  # 最後のモデルで、統合されていない信号線がある場合
                    ...
                    if output_data[0][j + 1] == correct_value[0]:  # 最後の出力ノードの値が0の場合
                        model_pass_fail[idx][line_id[j + 1 + count + suplit_data_sum_count]] = 0  # 統合されていない信号線の値をそのまま格納
                    elif output_data[0][j + 1] == correct_value[3]:  # 最後の出力ノードの値が1縮退故障の値の場合
                        model_pass_fail[idx][line_id[j + 1 + count + suplit_data_sum_count]] = 1

# モデルの学習に時間が掛かる大きな回路（モデルの分割数が多く、学習に時間が掛かる）において、正解率を確認するためだけに、モデルをprocess個（1回に並列学習できるモデル分）だけ学習させるプログラム
# ann_learn2.pyを修正したもの
# cd workspace/research2/experiment
#　実行コマンド：　python3 ann_learn11.py
...
epochs = 1
batch_size = 4
model_folder = cir + 'sepmodel_check'  # 学習済みモデルを保存するフォルダ
...
def build_and_train_model(input_data, correct_data, model_id):
    ...
    # モデルの構築
    model = Sequential()
    # # LeakyRelu以外を使用する場合のコード
    # model.add(Dense(mid_node_num, input_dim=input_node_num, activation='linear'))  # 入力層と中間層1
    # model.add(Dense(mid_node_num2, activation='linear'))  # 中間層2
    # model.add(Dense(output_node_num, activation='linear'))  # sigmoid or linear

    #  LeakyReluを使用する場合のコード
...
if __name__ == '__main__':
    ...
    # 学習時間短縮のために、processes個だけ学習させる
    num_models = processes

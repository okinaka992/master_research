...
#  故障候補を取得する関数
def get_fault_candidate(pass_fail, compare_model_pass_fail, fault_all_line, signal_num, br_missing_line, fault_target_type, fault_flag): # fault_flagは0なら縮退故障、1ならブリッジ故障を表す
    if fault_flag == 0:  # 縮退故障の診断を行う場合
        fault_type_sum = 2  # 縮退故障の種類は0縮退故障と1縮退故障の2種類
    else:  # ブリッジ故障の診断を行う場合
        fault_type_sum = 10  # ブリッジ故障の種類は0~9の10種類
    ...
    fault_type = [[] for _ in range(target_fault_num)] # 種類を格納するリスト 縮退故障の場合（0=0縮退故障、1=1縮退故障）。ブリッジ故障の場合（0~9=ブリッジ故障の種類）
    ...
    for i in range(target_fault_num):
        if all( x == 0 for x in pass_fail[i]) == True:  #区別できない故障の場合（＝全てのパスフェイルが0の場合）は、故障候補が無数にある可能性があるため、評価には使用しない
            ...
            print(f"fault_candidata：{fault_candidate[i]}")  # 故障候補が無数にあることを表示
            continue  # 次の故障に進む
        line_count = 0
        count = 0
        for j in range(signal_num*fault_type_sum):  # signal_numは、診断対象の回路における信号線の総数。fault_type_numは、縮退故障の場合は2、ブリッジ故障の場合は10
            if (compare_model_pass_fail[j] == pass_fail[i]) and (fault_target_type[i] == count):  # パスフェイル情報を比較
            ...
            count += 1
            if count == fault_type_sum:  # st_compare_model_pass_failは、2行ごとが0または1縮退故障の信号線に対応しているため、2行ごとにカウントをリセットする
                line_count += 1
                count = 0
...
def check_fault_candidate(fault_candidate, fault_type, fault_target_line, fault_target_type, fault_flag): # fault_flagは0なら縮退故障、1ならブリッジ故障を表す
    ...
    fault_candidate_sum = [0 for _ in range(fault_type_sum)]  # 故障候補の総数。故障の種類ごとに故障候補の総数を格納するリスト
    with open(correct_fault_candidate_file, 'w') as f:
        with open(incorrect_fault_candidate_file, 'w') as g:
            for i in range(target_fault_num):
                ...
                fault_candidate_sum[i] += len(fault_candidate[i])  # 故障候補の総数を更新
    ...
    print("平均故障候補数：" + str(fault_candidate_sum/(target_fault_num - indistinguishable_fault_count)))
    ...
    with open(diagnosis_rate_file, 'a') as f:
        ...
        f.write("平均故障候補数：" + str(fault_candidate_sum/(target_fault_num - indistinguishable_fault_count)) + '\n')
...
if __name__ == '__main__':
  ...
  # 学習済みの機械学習モデルに入力データを与えて、出力を取得する
    ...
    line_id = [0 for _ in range(signal_num - (signal_num%integration_num) + single_flag)]  # 故障信号線のIDを格納するリスト.統合されていない信号線がある場合を考慮して、single_flagを足している
    ...
    if single_flag == 1:  # 統合されていない信号線がある場合
        line_id[i*integration_num + j + 1] += int(lines[i+1][0]) # 最後の要素に統合されていない信号線のIDを追加
            ...
            if model_id == (num_models - 1) and single_flag == 1:  # 最後のモデルで、統合されていない信号線がある場合
                for j in range(output_node_num - 1):
                    if output_data[0][j] == correct_value[0]:  # 0縮退故障の値の場合
                        ...
                        model_pass_fail[i][line_id[j + count + integration_num*suplit_data_num*model_id]]= 1
                        model_pass_fail[i][line_id[j + count + integration_num*suplit_data_num*model_id + 1]] = 1
                    # temp[i][temp_count+suplit_data_num*model_id] = output_data[0][j]  # 故障出力値を格納
                    # temp_count += 1
                    count += 1
                if output_data[0][j + 1] == correct_value[0]:  # 最後の出力ノードの値が0の場合
                    model_pass_fail[i][line_id[j + 1 + count + integration_num*suplit_data_num*model_id]] = 0  # 統合されていない信号線の値をそのまま格納
                elif output_data[0][j + 1] == correct_value[3]:  # 最後の出力ノードの値が1縮退故障の値の場合
                    model_pass_fail[i][line_id[j + 1 + count + integration_num*suplit_data_num*model_id]] = 1
                # temp[i][temp_count+suplit_data_num*model_id] = output_data[0][j+1]
            else:  # 最後のモデルで、統合されていない信号線がない場合、または最後のモデル以外の場合
                for j in range(output_node_num):
  ...
  # モデル出力を故障の種類（fault_type_sum個）に分ける
    ...
    st_fault_candidate, st_fault_type = get_fault_candidate(st_pass_fail, st_compare_model_pass_fail, st_fault_all_line, signal_num, br_missing_line, st_fault_target_type, 0)  # 縮退故障の候補を取得(縮退故障の時、br_missing_lineは使わない、引数に設定しているから一応おいている)
    br_fault_candidate, br_fault_type = get_fault_candidate(br_pass_fail, br_compare_model_pass_fail, br_fault_all_line, signal_num, br_missing_line, br_fault_target_type, 1)  # ブリッジ故障の候補を取得

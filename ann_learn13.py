...
all_models_learn = False # 全てのモデルを学習させるかどうかのフラグ. True: 全てのモデルを学習させる, False: 1回にprocess個のモデルだけ学習させる
...
def mk_input_data():
    ...
    # int_linesの要素を先頭から任意の数削除
    num_to_remove = 150  # 削除したい要素数を指定
    int_lines = int_lines[num_to_remove:]

    global input_data_num
    input_data_num = int(len(int_lines))      #学習データ数を設定。学習データ数は入力データの行数
    global input_node_num               #グローバル変数を書き換え
    input_node_num = int(len(int_lines[0]))      #入力ノード数を設定。入力ノード数は入力データの各行の要素数

# 正解データをランダムな順序に分割するプログラム
# 正解データを0~1に正規化. 0~3の値を0~1に変換。0->0, 1->0.375, 2->0.625, 3->1 (※値がなるべく閾値から離れるように0は0のまま、3は1とした)
# cd workspace/research2/experiment
#　実行コマンド：　python3 learn_randomdata_suplit.py

#グローバル変数
cir = 's344'  # 対象回路
correct_data_file = cir + 'output'  # 正解データファイル
suplit_num = 21 # 何個ずつ分割するか
correct_data_folder = cir + 'ランダム分割正解データ' # 正解データを分割して保存するフォルダ
suplit_correct_data_file = cir + 'random_output'  # 分割された正解データファイル

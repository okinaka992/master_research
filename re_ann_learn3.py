...
def set_epochs(model_id):
    ...
    if add_mid_node_num == 0 and add_mid_node_num2 == 0:  # 中間層ノード数を追加していない場合 = エポック数だけを増やして正解率の改善を図る場合 ⇒　一度の実行で学習回数を追加した後のエポック数を参照したいため，「epoch_num_after_added_file」を読み込む
        with open(epoch_num_after_added_file) as f:
            epoch_nums = [int(line.strip()) for line in f.readlines() if line.strip()]
    else:                                                 # 中間層ノード数を追加している場合 = 中間ノードを増やして正解率の改善を図る場合（※エポック数も増やす）⇒初期のエポック数を参照したいため，「epoch_nums」を読み込む
        with open(epoch_num_file) as f:
            epoch_nums = [int(line.strip()) for line in f.readlines() if line.strip()]

    epochs = epoch_nums[model_id] + add_epoch_num

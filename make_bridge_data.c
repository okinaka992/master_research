...
char fltfile[MAXNAME], infile[MAXNAME], linfile[MAXNAME], ptnfile[MAXNAME], outfile1[MAXNAME], outfile3[MAXNAME], outf_c[MAXNAME], outn[MAXNAME], outfile[MAXNAME], o_f1[MAXNAME], o_f3[MAXNAME], indata[50], outdata[3000];

int main(int argc, char *argv[])
{
  int i, j, k, h, p_num, numptn, numlin, st_o, numout, numflt, id_n[MAXFAULT], g1[MAXFAULT], v[MAXFAULT], g2[MAXFAULT], c_lin_n, check_lin[50], flag2, count; //sa_num;
  char pt, id, f, lin_v[MMAXLINE], out_v[MAXLINE];
  ...
  */
  ...
  for(h = 0; h < 30; h++){
	...
	return 1;
      ...
      fscanf(f_flt, "%s %d %d\n", &pt, &p_num, &numflt);
      for(j = 0; j < numflt; j++){
	fscanf(f_flt, "%s %d %s %d %d %d\n", &id, &id_n[j], &f, &g1[j], &v[j], &g2[j]);
	if(id_n[j] == c_lin_n){
	  fault_lin = g1[j];
	  //sa_num = v[j];
	}
	...
	printf("\n");
      */
      ...
      flag2 = 0;
      for(k = 0; k < numout; k++){
	//if(out_v[c_lin_n][k] == lin_v[i][st_o+numout-2-k]){
    ...
    if(fault_lin+1 < st_o){
      fprintf(f_out3, "%d %d #fault_number testpattern_num", fault_lin, (c_lin_n%10)+2);
    }
    else if(fault_lin+1 >= st_o+numout){
      fprintf(f_out3, "%d %d #fault_number testpattern_num", fault_lin-numout, (c_lin_n%10)+2);
    }
    else{

                ...
                if model_id == (num_models - 1) and single_flag == 1:  # 最後のモデルで、統合されていない信号線がある場合
                    for j in range(output_node_num - 1):
                        if output_data[0][j] == correct_value[0]:  # 0縮退故障の値の場合
                            ...
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id]]= 0
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id + 1]] = 0
                        elif abs(output_data[0][j] - correct_value[1]) < 1e-2:  # floatの比較は、誤差を考慮してabsを使用
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id]]= 1
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id + 1]] = 0
                        elif abs(output_data[0][j] - correct_value[2]) < 1e-2:
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id]]= 0
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id + 1]] = 1
                        else:
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id]]= 1
                            model_pass_fail[idx][line_id[j + count + integration_num*suplit_data_num*model_id + 1]] = 1
                    ...
                    if output_data[0][j + 1] == correct_value[0]:  # 最後の出力ノードの値が0の場合
                        model_pass_fail[idx][line_id[j + 1 + count + integration_num*suplit_data_num*model_id]] = 0  # 統合されていない信号線の値をそのまま格納
                    elif output_data[0][j + 1] == correct_value[3]:  # 最後の出力ノードの値が1縮退故障の値の場合
                        model_pass_fail[idx][line_id[j + 1 + count + integration_num*suplit_data_num*model_id]] = 1
                    # temp[idx][temp_count+suplit_data_num*model_id] = output_data[0][j+1]
                else:  # 最後のモデルで、統合されていない信号線がない場合、または最後のモデル以外の場合
                    for j in range(output_node_num):

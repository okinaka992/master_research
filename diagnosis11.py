...
target_fault_num = 1  # 故障診断対象の故障の数⇒全ての故障の中からtarget_fault_num個をランダムに選択する
    ...
    for model_id in range(num_models):
            ...
            for integ_num in range(len(correct_data_integrated_inf[i])):
                ...
                if model_id == single_line_model and single_flag == 1:  # 統合されていない信号線があり，統合されていない信号線を学習したモデルの場合
                    ...
                    for j in range(output_node_num):
                        ...
                        # temp[idx][temp_count+suplit_data_num*model_id] = output_data[0][j]  # 故障出力値を格納
                        # temp_count += 1
        # バッチ推論のみを使用（逐次ループは削除済み）
        n = 0
        correct_data_integrated_inf_count = 0  # correct_data_integrated_infのカウント変数

        # --- ここからバッチ推論(一度に最大10サンプル) ---
        batch_size = 10
        N = suplited_input_data_num
        input_shape = list(input_details[0]['shape'])  # 例: [1, feat_dim]
        feat_shape = tuple(input_shape[1:]) if len(input_shape) > 1 else ()
        orig_input_shape = list(input_shape)
        ...
        for start in range(0, N, batch_size):
            ...
            if feat_shape:
                ...
                for k in range(b):
                    batch_inputs[k] = np.reshape(input_data[start + k], feat_shape)
            else:
                batch_inputs = np.zeros((b,), dtype=np.float32)
                for k in range(b):
                    batch_inputs[k] = input_data[start + k]
            ...
            for k in range(b):
                i = start + k
                output_data = outputs[k:k+1]  # shape (1, M)

                # 出力分類（既存ロジック）
                if (model_id == single_line_model) and single_flag == 1:
                    for j in range(output_node_num):
                        if j != single_line_suplit_idx:
                            if output_data[0][j] < threshold[0]:
                                output_data[0][j] = correct_value[0]
                            elif output_data[0][j] < threshold[1]:
                                output_data[0][j] = correct_value[1]
                ...
                # 統合解除と model_pass_fail への格納（境界チェックを追加）
                for integ_num in range(len(correct_data_integrated_inf[i])):
                    count = 0
                    idx = int(correct_data_integrated_inf[i][integ_num])
                    if idx < 0 or idx >= len(model_pass_fail):
                        # 不正な idx はスキップ
                        continue
                    if model_id == single_line_model and single_flag == 1:
                        for j in range(output_node_num):
                            target_pos = j + count + suplit_data_sum_count
                            if target_pos < 0 or target_pos >= len(line_id):
                                # 範囲外ならスキップ（ログ）
                                # print(f"警告: target_pos out of range: {target_pos} (len(line_id)={len(line_id)})")
                                count += 1
                                continue
                            col = line_id[target_pos]
                            if col < 0 or col >= len(model_pass_fail[idx]):
                                count += 1
                                continue
                            if output_data[0][j] == correct_value[0]:
                                model_pass_fail[idx][col] = 0
                                # 次の列が存在するかチェックして設定
                                if (target_pos + 1) < len(line_id):
                                    col2 = line_id[target_pos + 1]
                                    if 0 <= col2 < len(model_pass_fail[idx]):
                                        model_pass_fail[idx][col2] = 0
                            elif abs(output_data[0][j] - correct_value[1]) < 1e-2:
                                model_pass_fail[idx][col] = 1
                                if (target_pos + 1) < len(line_id):
                                    col2 = line_id[target_pos + 1]
                                    if 0 <= col2 < len(model_pass_fail[idx]):
                                        model_pass_fail[idx][col2] = 0
        ...
        # print(output_data)                # output_dataには、テストパターンnのときの、故障があるかどうかの情報が格納されている
        if model_id == single_line_model and single_flag == 1:
            suplit_data_sum_count += integration_num*output_node_num - 1  # 処理された正解データの総数を更新
        else:

# ann_learn2.pyとann_learn11.pyを合体したプログラム
...
correct_data_file = cir + '分割正解データ削除後2' + '/' + cir + 'integrated_output'  # 統合後の正解データファイル
correct_data_value_file = cir + '分割正解データ2' + '/' + cir + 'correct_value'  # 正解データの種類（値）を保存するファイル
threshold_file = cir + '分割正解データ2' + '/' + cir + 'threshold'  # 閾値を保存するファイル
suplit_num_file = cir + '分割正解データ2' + '/' + cir + 'suplit_num'  # 分割数が保存されたファイル
single_line_file = cir + '分割正解データ2' + '/' + cir + 'single_line'  # 統合されていない信号線があるかが保存されたファイル
...
mid_node_num = 50  #生成器の中間層のノード数　初期値は128　学習結果によって変更
mid_node_num2 = 100  #生成器の中間層のノード数　初期値は128　学習結果によって変更

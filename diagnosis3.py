...
def get_fault_candidate(pass_fail, compare_model_pass_fail, fault_all_line, signal_num, br_missing_line, fault_flag): # fault_flagは0なら縮退故障、1ならブリッジ故障を表す
    ...
    # 縮退故障のパスフェイル情報を比較
    fault_type_candidate = [[] for _ in range(target_fault_num)] # 故障候補を格納するリスト（故障の種類ごと）
    ...
    for i in range(target_fault_num):
        if all( x == 0 for x in pass_fail[i]) == True:  #区別できない故障の場合（＝全てのパスフェイルが0の場合）は、故障候補が無数にある可能性があるため、評価には使用しない
            fault_type_candidate[i].append(-1)  # 故障候補に-1を設定
            fault_type[i].append(-1)
            print(f"fault_candidata：{fault_type_candidate[i]}")  # 故障候補が無数にあることを表示
            continue  # 次の故障に進む
        line_count = 0
        count = 0
        for j in range(signal_num*fault_type_num):  # signal_numは、診断対象の回路における信号線の総数。fault_type_numは、縮退故障の場合は2、ブリッジ故障の場合は10
            if compare_model_pass_fail[j] == pass_fail[i]:  # パスフェイル情報を比較
                ...
                fault_type_candidate[i].append(fault_all_line[line_count])  # 故障候補を格納するリストに追加
        ...
        if len(fault_type_candidate[i]) == 0:  # 故障候補が無い場合は、0を設定
            fault_type_candidate[i].append(0)  # 故障候補が無い場合は、0を設定
            fault_type[i].append(0)  # 故障の種類も0を設定
        print(f"fault_type_candidate[{i}]: {len(fault_type_candidate[i])}")  # 故障候補数を表示
    
    return fault_type_candidate, fault_type  # 故障候補と故障の種類を返す


# 故障候補の中に、診断対象の故障信号線が含まれているかを確認する関数
def check_fault_candidate(fault_type_candidate, fault_type, fault_target_line, fault_target_type, fault_flag): # fault_flagは0なら縮退故障、1ならブリッジ故障を表す
    if fault_flag == 0:  # 縮退故障の診断を行う場合
    ...
    fault_candidate_sum = 0  # 故障候補の総数
    with open(correct_fault_candidate_file, 'w') as f:
        with open(incorrect_fault_candidate_file, 'w') as g:
            for i in range(target_fault_num):
                fault_type_candidate[i] = sorted(fault_type_candidate[i]) # 小さい番号順に並び替える
                if fault_type_candidate[i] == [-1]:  # 故障候補が無数にある場合は、評価には使用しない
                    ...
                    continue  # 次の故障に進む
                if fault_type_candidate[i] == [0]: # 故障候補が無い場合は、0を設定
                ...
                fault_candidate_sum += len(fault_type_candidate[i])  # 故障候補の総数を更新
                for j in range(len(fault_type_candidate[i])):
                    if fault_type_candidate[i][j] == fault_target_line[i] and fault_type[i][j] == fault_target_type[i]:
                        find_count += 1
                        print(f"{fault_name}候補の中に実際に選んだ{fault_name}番号 {fault_type_candidate[i][j]} の {fault_type[i][j]} {fault_name}が含まれていました")
                        f.write("故障候補数；" + str(len(fault_type_candidate[i])) + '\n')
                        f.write(str(fault_type_candidate[i][j]) + fault_symbol + str(fault_type[i][j]) + '\n')
                        break
                    if j == len(fault_type_candidate[i]) - 1:

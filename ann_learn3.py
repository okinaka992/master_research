# ann_learn2のプログラムを乱数を用いて１０回学習を行うように設定
# 複数のANNの学習を並列に行うプログラム
# cd workspace/research2/experiment
#　実行コマンド：　python3 ann_learn3.py
...
if __name__ == '__main__':
    ...
    # 学習させるモデルの数 = 分割されたデータの数を取得
    with open(suplit_num_file, 'r') as f:
        num_models = int(f.readline())
    
    for run_id in range(num_runs):  # 10回繰り返す
        print(f'学習の実行: {run_id + 1}/{num_runs}')
        
        # 擬似乱数シードの設定
        seed = 42 + run_id  # 基本シード値に実行回数を加える
        np.random.seed(seed)
        tf.random.set_seed(seed)

        # プロセスプールの作成
        p = multiprocessing.Pool(processes=processes)
        result = p.map(worker, range(0, num_models))
        p.close()
        p.join()

        # 結果を記録
        accuracy_file = f'{cir}_run{run_id + 1}_model_accuracy.txt'
        with open(accuracy_file, 'w') as f:
            for model_id, acc in enumerate(result):
                f.write(f'Run {run_id + 1} - モデル{model_id}: 正解率 {acc:.4f}\n')

        print(f'学習の実行: {run_id + 1}/{num_runs} 完了')

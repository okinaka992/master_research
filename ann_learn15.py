# ann_learn14.pyを編集したプログラム。モデルの中間層のノードを正解データ数に応じて変更した場合でも、学習と評価ができるようにした。
...
middle_layer_node_num_file = cir + '分割正解データ削除後' + '/' + cir + 'middle_layer_node_num'  # 各正解データごとの中間層ノード数が保存されたファイル
delited_data_num_file = cir + '分割正解データ削除後' + '/' + cir + 'delited_data_num'  # 各分割正解データごとに削除された正解データ数が保存されたファイル

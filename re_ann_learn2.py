...
add_mid_node_num = 0  # 追加する中間層ノード数
add_mid_node_num2 = 0  # 追加する中間層ノード数2
...
max_re_learning_count = 1  # 最大再学習回数
...
if __name__ == '__main__':
    ...
    msg['from'] = os.getenv('MAIL_FROM', 'm806002y@mails.cc.ehime-u.ac.jp')  # 送信元メールアドレスを設定
    msg['to'] = os.getenv('MAIL_TO', 'm806002y@mails.cc.ehime-u.ac.jp')  # 送信先メールアドレスを設定
    msg.set_content(os.path.basename(__file__) + 'のプログラムが終了しました。\n\n' \
                    '回路名：' + cir + '\n' \
                    '再学習回数：' + str(re_learning_count) + '\n' \
                    '実行プログラム：' + os.path.basename(__file__) + '\n' \
                    '日時：' + datetime.strftime("%Y-%m-%d %H:%M:%S") + '\n')  # メール本文を設定
    ...
    if stmp_server: # 環境変数で外部SMTPが指定されている場合（認証あり）
        ...
        try:
            ...
            err_log_dir = error_output_file or '.'
            os.makedirs(err_log_dir, exist_ok=True)
            log_path = os.path.join(err_log_dir, f'{cir}_mail_fallback_error.log')
            with open(log_path, 'a') as lf:
                lf.write(f"{datetime.strftime('%Y-%m-%d %H:%M:%S')} mail send failed: {e}\n")
            print("メール送信に失敗しました（sendmail/localhost SMTP 利用不可）。詳細は:", log_path)
    ...
    def _log_mail_error(e): # メール送信エラーをログに記録する内部関数
        err_log_dir = error_output_file or '.'
        os.makedirs(err_log_dir, exist_ok=True)
        log_path = os.path.join(err_log_dir, f'{cir}_mail_error.log')
        with open(log_path, 'a') as lf:
            lf.write(f"{datetime.strftime('%Y-%m-%d %H:%M:%S')} mail send failed: {e}\n")
